...
function loadTryAPL () {
	...
	if (typeof(Storage) !== undefined) {
		if (localStorage.current !== undefined && canSave) {
			...
			// Dark mode
			html.className = localStorage.getItem("darkMode")
		}
	}
	else {
...
setCanSave=cs=>{
	...
	canSave ? localStorage.setItem("canSave", canSave) : localStorage.clear()
}

permaLink=_=>{
	currentLine = getSessionInput(0)
	history.replaceState({},document.title,location.pathname + "?clear&q=" + encodeURIComponent(currentLine.trim()) + "&run")
}
